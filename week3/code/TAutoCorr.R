...
sresid <- residuals(lm(df$temp1 ~ df$temp0))^2
acf(sresid, main = "Auto-cor")
...
# ##########################
# 
# # Obs: In this simulation we fixed the initial temperatures and just shuffle
# # the second vector, for this we adjust the function
# 
# # Function to calculate the autocorrelation value between years
# find_autocorr_v2 <- function(v){
#   #temp0 <- head(v, length(v)-1)
#   temp1 <- tail(v, length(v)-1)
# 
#   corfit <- cor(temp0, temp1)
...
# 
# for (i in 1:n_iters){
#   temp_cor[i] <- find_autocorr_v2(sample(ats$Temp, replace = FALSE))
# }
# hist(temp_cor, breaks = 30)
